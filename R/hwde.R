"hwde" <-
function(data=IndianIrish, gp="Population", termlist=NULL, refmodel=NULL, 
             loci=paste("locus",1:(dim(data)[2]-1),sep=""),
             observed="Observed", keep.models=FALSE, aovtable.print=TRUE,
             group.terms=TRUE, allele.chars=letters){
            ## Start tmaker()
       tmaker <- function(trms=c("ma","mb"), currbase=1, group.terms=TRUE,
            ...
            if (group.terms)
            {plus <- paste("(",paste(trms,collapse="+"),")",sep="")
             refmodel <- currbase
         } else
        {  plus <- paste(pref,
                         c(trms,paste("(",paste(trms,collapse="+"),
                                      ")",sep="")),sep="")
        ...
        nobs <- length(obs)
        gploc <- loci
        contrasts.info <- make.contrasts(data=data[,loci],
                                         allele.chars=allele.chars)
        contr.df <- contrasts.info$contrasts.df
        list.columns <- contrasts.info$list.columns
        if(!is.null(data[,gp]))
        data.df <- cbind(data.frame(obs=obs,gp=data[,gp]), contr.df) else
        data.df <- cbind(data.frame(obs=obs), contr.df)
        ...
        if(is.null(termlist)){
            ...
            {
                trms <- tmaker(colvec,currbase=newbase,
    ...
    {addterms <- termlist
        ...
        if(!group.terms)nam[1] <- paste("r",nam[1],sep="")
        ...
        invisible(list(anovatab=anovatab, data.df=data.df,
                       aovtab.terms=aovtab.terms)) else
        invisible(list(anovatab=anovatab, data.df=data.df,
